...
from modules.lora import LoRALinearLayer, LoraLoaderMixin
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="The output directory where the pretrained model predictions and checkpoints has be written.",
    )
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if not os.path.isfile(instance_image_path):
            raise ValueError('%s is not a file!'%instance_image_path)

        self.instance_images_path = [instance_image_path]
...
def main(args):
    ...
    # TODO: RESUME
    if args.resume_dir:
        lora_state_dict, network_alphas = LoraLoaderMixin.lora_state_dict(args.resume_dir)
    ...
    for i, (attn_processor_name, attn_processor) in enumerate(unet.attn_processors.items()):
        ...
        for layer_name in ['to_q', 'to_k', 'to_v', 'to_out']:
            ...
            layer.down.weight.data.copy_(lora_state_dict[down_key].to(torch.float32))
            layer.up.weight.data.copy_(lora_state_dict[up_key].to(torch.float32))
    ...
    if args.train_text_encoder:
        ...
        text_lora_parameters = LoraLoaderMixin._modify_text_encoder(text_encoder, lora_state_dict, dtype=torch.float32, rank=args.rank, patch_mlp=args.patch_mlp)
    ...
    train_dataset = DreamBoothDataset(
        instance_image_path=args.instance_image_path,
    ...
    if args.max_train_steps is None:
        args.max_train_steps = args.num_train_steps * num_update_steps_per_epoch
    ...
    if accelerator.is_main_process:
        ...
        print("unet_lora_layers>>>>>>>>",len(unet_lora_layers))
        print("text_encoder_lora_layers>>>>>",len(text_encoder_lora_layers))
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            ...
            for i,image in enumerate(images):
                image.save("aigc_samples/validation_%d.jpg"%i)

            for tracker in accelerator.trackers:
                if tracker.name == "tensorboard":
                    np_images = np.stack([np.asarray(img) for img in images])
                    tracker.writer.add_images("test", np_images, step, dataformats="NHWC")
