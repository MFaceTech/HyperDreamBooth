...
class LoRALinearLayer(nn.Module):
    def __init__(self, in_features, out_features, rank=4, network_alpha=None, device=None, dtype=None):
        ...
        self.rank = rank  # trainable lora

        if rank > min(in_features, out_features):
            raise ValueError(f"Frozen LoRA rank {rank} must be less or equal than {min(in_features, out_features)}")
        ...
        self.down = nn.Linear(in_features, rank_f, bias=False, device=device, dtype=dtype)
        self.up = nn.Linear(rank_f, out_features, bias=False, device=device, dtype=dtype)
    ...
    def convert_to_standard_lora(self):
        ...
        with torch.no_grad():
            ...
            new_weight = torch.mm(self.up.weight, self.down.weight)
            # torch.svd
            ...
            #  Remove abandoned layers and variables
            del self.down, self.up

            # Redefine down and up layers
