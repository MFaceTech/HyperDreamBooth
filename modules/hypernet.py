...
from .light_lora import LoRALinearLayer
...
class ImgWeightGenerator(nn.Module):
    def __init__(
        ...
        if platform.system().lower() == "linux":
            model_dir = "projects/AIGC/model_zoo"
        elif platform.system().lower() == "darwin":
            model_dir = "workspace/model_zoo"
        else:
            raise ValueError('Operating system does not support!')

        # vit encoder
        print("encoder_model_name: ", encoder_model_name)
        if encoder_model_name == "vit_base_patch16_224":
            # forward_features: [1, 197, 768]
            checkpoint_path = os.path.join(model_dir, "B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_224.npz")
        elif encoder_model_name == "models--timm--vit_huge_patch14_clip_224.laion2b_ft_in12k_in1k.safetensors":
            # forward_features: [1, 257, 1280]
            checkpoint_path = os.path.join(model_dir, "models--timm--vit_huge_patch14_clip_224.laion2b_ft_in12k_in1k.safetensors")
        elif encoder_model_name == "vit_huge_patch14_clip_336.laion2b_ft_in12k_in1k":
            # forward_features: [1, 577, 1280]
            checkpoint_path = os.path.join(model_dir, "models--timm--vit_huge_patch14_clip_336.laion2b_ft_in12k_in1k.safetensors")
        else:
            raise ValueError('%s does not exist!'%encoder_model_name)
        # Create ViT Model
        self.encoder_model: nn.Module = create_model(encoder_model_name, checkpoint_path=checkpoint_path)
        ...
        test_input = torch.randn(1, 3, reference_size, reference_size)
    ...
    def encode_features(self, ref_img):
        # ref_img = resize(ref_img, [self.ref_size, self.ref_size], antialias=True)
...
class HyperDream(nn.Module):
    def __init__(
        ...
        self.liloras: Dict[str, LoRALinearLayer] = {}
    ...
    def set_lilora(self, liloras):
        ...
        print(f"Total LiLoRAs: {length}, Hypernet params for each image: {length * self.weight_dim}")
...
class PreOptHyperDream(nn.Module):
    ...
    def set_lilora(self, liloras, identities=1):
        ...
        length = len(self.liloras_keys)
        print(f"Total LiLoRAs: {length}, Pre-Optimized params for each image: {length * self.params_per_lora}")
    ...
    def gen_weight(self, identities: torch.Tensor):
        ...
        weight_list = weights.split(1, dim=1)  # [b, n, dim] -> n*[b, 1, dim] -> n*[b, dim]
        return weights, [weight.squeeze(1) for weight in weight_list]

    def forward(self, identities: torch.Tensor):
        if self.training and self.gradient_checkpointing:
            weights, weight_list = checkpoint.checkpoint(
                self.gen_weight, identities
            )
        else:
            weights, weight_list = self.gen_weight(identities)
