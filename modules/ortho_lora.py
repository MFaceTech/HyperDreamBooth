...
class CustomLinear(nn.Module):
    """
    权重矩阵来自外部输入，如果权重矩阵是2D的，那么所有的样本都将共享同一个权重矩阵。如果权重矩阵是3D的，那么每个样本都有自己的权重矩阵。
    """
    ...
    def forward(self, input, weight=None):
        # Check the dimensions of the external weight matrix
        if weight.dim() == 2:
            ...
            assert weight.shape == (input.shape[0], self.out_features, self.in_features)
            return torch.matmul(input, weight.transpose(1, 2))
        else:
            # Raise an error if the weight matrix is neither 2D nor 3D
            raise ValueError('The external weight matrix must be 2D or 3D')
...
class LoRALinearLayer(nn.Module):
    def __init__(
        ...
        self.up_dim = up_dim
        self.rank = rank
        self.is_train = is_train  # trainable for internal weight_embedding
        ...
        # Define the four Custom Linear layers for OrthoDecompLora
        self.down_layer = CustomLinear(in_features, rank)
        self.up_layer = CustomLinear(rank, out_features)

        if is_train:
            # weight embedding initialization
            down_weight = torch.empty(rank, down_dim)
            up_weight = torch.empty(up_dim, rank)
            nn.init.normal_(down_weight, std=1/rank)
            # nn.init.normal_(up_weight)
            nn.init.zeros_(up_weight)
            self.weight_embedding = nn.Parameter(torch.cat((down_weight.view(-1), up_weight.view(-1))))


    def update_aux(self, aux_seed=None, down_aux=None, up_aux=None):
        """
        Update the auxiliary parameters of LoRA.
        This function is called when restoring the weights of LoRa from a checkpoint.
        """
    ...
    def update_weight(self, weight_embedding: torch.Tensor):
        ...
        if len(weight_embedding.shape) > 2 or (
            ...
            raise ValueError(
                "The shape of weight_embedding must be [batch_size, (up_dim+down_dim)*r] or [(up_dim+down_dim)*r], and the dimension of weight_embedding must not be more than 2.")
        self.weight_embedding = weight_embedding
        # print("weight_embedding",weight_embedding.shape)
    ...
    def convert_to_standard_lora(self):
        ...
        # merge weight
        down = torch.matmul(down_weight, down_aux)
        up = torch.matmul(up_aux, up_weight)
        # convert to linear weight
        with torch.no_grad():
            self.down = nn.Linear(self.in_features, self.rank, bias=False, device=self.device, dtype=self.dtype)
            self.up = nn.Linear(self.rank, self.out_features, bias=False, device=self.device, dtype=self.dtype)
            self.down.weight = nn.Parameter(down)
            self.up.weight = nn.Parameter(up)

        # remove reduntant variables
        del self.down_layer, self.up_layer
    ...
    def forward(self, hidden_states):
        ...
        up = torch.matmul(up_aux, up_weight)
        # linear layer
        down_hidden_states = self.down_layer(hidden_states.to(dtype), down)
        up_hidden_states = self.up_layer(down_hidden_states, up)
