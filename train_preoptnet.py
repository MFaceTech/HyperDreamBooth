...
def main(args):
    ...
    for i, (attn_processor_name, attn_processor) in enumerate(unet.attn_processors.items()):
        ...
        attn_module.to_q.set_lora_layer(
            LoRALinearLayer(
                in_features=attn_module.to_q.in_features, out_features=attn_module.to_q.out_features, rank=args.rank, is_train=False
            )
        )
        attn_module.to_k.set_lora_layer(
            LoRALinearLayer(
                in_features=attn_module.to_k.in_features, out_features=attn_module.to_k.out_features, rank=args.rank, is_train=False
            )
        )
        attn_module.to_v.set_lora_layer(
            LoRALinearLayer(
                in_features=attn_module.to_v.in_features, out_features=attn_module.to_v.out_features, rank=args.rank, is_train=False
            )
        )
    ...
    # a reference needs to be passed here.
    ...
    # [TODO] model hook for hyperdreambooth
    ...
    def load_model_hook(models, input_dir):
        ...
        # LoraLoaderMixin.load_lora_into_text_encoder(lora_state_dict, network_alphas=network_alphas, text_encoder=text_encoder_)
    ...
    # TODO: HyperNetwork defined here!
    hypernetwork = PreOptHyperDream(
        args.rank,
        args.down_dim,
        args.up_dim,
    ).to(weight_dtype)
    hypernetwork.set_lilora(lora_linear_layers, train_dataset.num_instance_images)
    ...
    for batch_num, batch in enumerate(train_dataloader):
        ...
        resume_batch = global_step / (args.train_steps_per_identity * args.gradient_accumulation_steps)
        if args.resume_from_checkpoint and batch_num < resume_batch:
            ...
            with accelerator.accumulate(unet):
                ...
                if args.pre_compute_text_embeddings:
                    if args.train_text_encoder:
                        ...
                        prob = torch.rand((encoder_hidden_states.size(0), 1, 1)).expand_as(encoder_hidden_states).to(accelerator.device)
                        encoder_hidden_states = torch.where(prob > args.cfg_drop_rate, encoder_hidden_states, empty_hidden_states)
                else:
                    prob = torch.rand((batch["input_ids"].size(0), 1)).expand_as(batch["input_ids"]).to(accelerator.device)
                    batch_input_ids = torch.where(prob > args.cfg_drop_rate, batch['input_ids'], batch['empty_input_ids'])
