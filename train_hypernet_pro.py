...
from modules.light_lora_pro import LoRALinearLayer, LoraLoaderMixin
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="The input directory where the preoptnet model is in.",
    )
    parser.add_argument(
        "--validation_input_dir",
        type=str,
        default="",
        help="The input directory where the validation images are in.",
    )
    parser.add_argument(
        "--validation_output_dir",
        type=str,
        default="",
        help="The output directory where the generated validation iamges will be saved.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="CFG training drop rate.",
    )
    parser.add_argument(
        "--pre_opt_weight_coeff",
        type=float,
        default=0.0,
        help="The pre_opt_weight loss cofficient",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=160,
        help=("The dimension of the LoRA down matrices."),
    )
    parser.add_argument(
        "--up_dim",
        type=int,
        default=80,
        help=("The dimension of the LoRA up matrices."),
    )
    ...
    assert args.pre_opt_weight_coeff == 0, "pre_opt_weight_coeff must be 0 when lora's aux matrix is learnable"

    return args


class HyperDreamBoothDataset(Dataset):
    """
    ...
    def __init__(
            ...
            hypernet_transforms=None,
    ):
        self.size = size
        self.center_crop = center_crop
        self.tokenizer = tokenizer
        self.encoder_hidden_states = encoder_hidden_states
        self.empty_hidden_states = empty_hidden_states
        self.class_prompt_encoder_hidden_states = class_prompt_encoder_hidden_states
        self.tokenizer_max_length = tokenizer_max_length
        self.hypernet_transforms = hypernet_transforms
        ...
        self.path_to_idx = {
            str(path): idx
            for idx, path in enumerate(sorted(self.instance_images_path))
        }
    ...
    def __getitem__(self, index):
        ...
        example["hypernet_inputs"] = self.hypernet_transforms(instance_image)

        if self.encoder_hidden_states is not None:
            example["instance_prompt_ids"] = self.encoder_hidden_states
            example["empty_prompt_ids"] = self.empty_hidden_states
        else:
            text_inputs = tokenize_prompt(
                self.tokenizer, self.instance_prompt, tokenizer_max_length=self.tokenizer_max_length
            )
            empty_inputs = tokenize_prompt(
                self.tokenizer, "", tokenizer_max_length=self.tokenizer_max_length
            )
            example["instance_prompt_ids"] = text_inputs.input_ids
            example["empty_prompt_ids"] = empty_inputs.input_ids
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    hypernet_inputs = [example["hypernet_inputs"] for example in examples]
    ...
    hypernet_inputs = torch.stack(hypernet_inputs)
    hypernet_inputs = hypernet_inputs.to(memory_format=torch.contiguous_format).float()

    input_ids = torch.cat(input_ids, dim=0)
    empty_input_ids = torch.cat(empty_input_ids, dim=0)

    batch = {
        "ids": [example["instance_ids"] for example in examples],
        "input_ids": input_ids,
        "empty_input_ids": empty_input_ids,
        "pixel_values": pixel_values,
        "hypernet_inputs": hypernet_inputs
    }
...
def main(args):
    ...
    # Set correct lora layers, the total layer is 32, total lora is 32*4=128
    ...
    if args.vit_model_name == "vit_base_patch16_224":
        ...
        raise ValueError("%s does not supports!" % args.vit_model_name)
    ...
    hypernet_transposes_test = Compose([
        Resize(size=ref_img_size),
        CenterCrop(size=(ref_img_size, ref_img_size)),
        ToTensor(),
        Normalize(mean=mean, std=std),
        ...
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(model, type(accelerator.unwrap_model(hypernetwork))):
                    ...
                    text_encoder_lora_layers_to_save = text_encoder_lora_state_dict(model, patch_mlp=args.patch_mlp)
                else:
            ...
            for i, lilora in enumerate(lora_linear_layers):
                state_dict['aux_seed_%d' % i] = lilora.aux_seed
                state_dict['down_aux_%d' % i] = lilora.down_aux.data
                state_dict['up_aux_%d' % i] = lilora.up_aux.data
    ...
    def load_model_hook(models, input_dir):
        ...
        for i, lilora in enumerate(lora_linear_layers):
            aux_seed = weight['aux_seed_%d' % i]
            down_aux = weight['down_aux_%d' % i]
            up_aux = weight['up_aux_%d' % i]
            lilora.update_aux(aux_seed, down_aux, up_aux)  # do not need initializations
    ...
    # Optimizer creation
    params_to_optimize = (
        itertools.chain(hypernetwork.parameters(), unet_lora_parameters, text_lora_parameters)
        if args.train_text_encoder
        else itertools.chain(hypernetwork.parameters(), unet_lora_parameters)
    )
    # print("params_to_optimize:", list(itertools.chain(hypernetwork.parameters(), unet_lora_parameters, text_lora_parameters)))
    ...
    if args.pre_compute_text_embeddings:
        ...
        pre_computed_empty_hidden_states = None
        validation_prompt_encoder_hidden_states = None
        validation_prompt_negative_prompt_embeds = None
        pre_computed_class_prompt_encoder_hidden_states = None

    # Dataset and DataLoaders creation:
    train_dataset = HyperDreamBoothDataset(
        ...
        empty_hidden_states=pre_computed_empty_hidden_states,
        class_prompt_encoder_hidden_states=pre_computed_class_prompt_encoder_hidden_states,
        tokenizer_max_length=args.tokenizer_max_length,
        hypernet_transforms=hypernet_transposes
    )
    ...
    # image preprocess for ViT image encoder

    # generated validation images.
    validation_images = []
    validation_image_paths = [os.path.join(args.validation_input_dir, f) for f in os.listdir(args.validation_input_dir)
                              if os.path.isfile(os.path.join(args.validation_input_dir, f))]
    validation_image_paths = sorted(validation_image_paths)
    for validation_image_path in validation_image_paths:
        validation_img = Image.open(validation_image_path).convert("RGB")
        validation_img = hypernet_transposes_test(validation_img).unsqueeze(0)
        validation_images.append(validation_img)
    if len(validation_images) > 0:
        validation_images = torch.cat(validation_images, dim=0).to("cuda")
    else:
        validation_images = None

    # TODO: Load PreOptHyperNet Weight
    if args.pre_opt_weight_path != '':
        pre_opt_hypernet = PreOptHyperDream(args.rank, args.down_dim, args.up_dim)
        pre_opt_hypernet.set_lilora(lora_linear_layers, train_dataset.num_instance_images)
        ...
        for i, lilora in enumerate(lora_linear_layers):
            ...
            down_aux = weight['down_aux_%d' % i]
            up_aux = weight['up_aux_%d' % i]
            lilora.update_aux(aux_seed, down_aux, up_aux)
        logger.info(f"Pre-optimized weights loaded from {args.pre_opt_weight_path}.")
    ...
    if args.train_text_encoder:
        unet, hypernetwork, text_encoder, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            unet, hypernetwork, text_encoder, optimizer, train_dataloader, lr_scheduler
        )
    else:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        unet.train()
        hypernetwork.train()
        hypernetwork.img_weight_generator.encoder_model.eval()
        hypernetwork.img_weight_generator.decoder_model.train()
        if args.train_text_encoder:
            text_encoder.train()
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                pixel_values = batch["pixel_values"].to(dtype=weight_dtype)
                hypernet_inputs = batch["hypernet_inputs"].to(dtype=weight_dtype)

                with accelerator.autocast():
                    # hypernet prediction
                    pred_weights, pred_weights_list = hypernetwork(hypernet_inputs)
                    # update weight for loras
                    for weight, lora_layer in zip(pred_weights_list, lora_linear_layers):
                        lora_layer.update_weight(weight)
                    with torch.no_grad():
                        # get pre-optimized weights according to identity
                        pre_opt_weights, pre_opt_weights_list = pre_opt_hypernet.gen_weight(batch['ids'])
                ...
                # Get the text embedding for conditioning: CFG training
                if args.pre_compute_text_embeddings:
                    if args.train_text_encoder:
                        raise ValueError("Cannot pre-compute text embeddings while training text encoder")
                    else:
                        encoder_hidden_states = batch["input_ids"]
                        empty_hidden_states = batch["empty_input_ids"]
                        prob = torch.rand((encoder_hidden_states.size(0), 1, 1)).expand_as(
                            encoder_hidden_states).to(accelerator.device)
                        encoder_hidden_states = torch.where(prob > args.cfg_drop_rate, encoder_hidden_states,
                                                            empty_hidden_states)
                else:
                    prob = torch.rand((batch["input_ids"].size(0), 1)).expand_as(batch["input_ids"]).to(
                        accelerator.device)
                    batch_input_ids = torch.where(prob > args.cfg_drop_rate, batch['input_ids'],
                                                  batch['empty_input_ids'])
                ...
                if args.with_prior_preservation:
                    ...
                    diffusion_loss = instance_loss + args.prior_loss_weight * prior_loss
                else:
                    diffusion_loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")

                # total loss
                loss = diffusion_loss + weight_loss * args.pre_opt_weight_coeff

                accelerator.backward(loss)
                if accelerator.sync_gradients:
                    params_to_clip = (
                        itertools.chain(hypernetwork.train_params(), unet_lora_parameters, text_lora_parameters)
                        if args.train_text_encoder
                        else itertools.chain(hypernetwork.train_params(), unet_lora_parameters)
                    )
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and validation_images is not None and epoch % args.validation_epochs == 0:
                logger.info(
                ...
                weights_list = []
                hypernetwork.eval()
                for batch in torch.split(validation_images, args.train_batch_size):
                    weight, _ = hypernetwork(batch)
                    weights_list.append(weight)
                batch_weights = torch.cat(weights_list, dim=0)  # [batch_size, weight_num, weight_dim]
                weights_list = torch.split(batch_weights, split_size_or_sections=1, dim=0)
                weights_list = [weight.squeeze(0) for weight in weights_list]  # batch_size * [weight_num, weight_dim]
                ...
                images = []
                num_validation_images = validation_images.size(0)
                for i in range(num_validation_images):
                    weights = weights_list[i]  # [weight_num, weight_dim]
                    finename = os.path.split(validation_image_paths[i])[-1]
                    with torch.cuda.amp.autocast():
                        for weight, lora_layer in zip(weights, lora_linear_layers):
                            lora_layer.update_weight(weight)
                        image = pipeline(**pipeline_args, generator=generator).images[0]
                        images.append(image)
                        save_dir = os.path.join(args.validation_output_dir, "Epoch_%d" % epoch)
                        os.makedirs(save_dir, exist_ok=True)
                        image.save(os.path.join(save_dir, finename))
    ...
    if accelerator.is_main_process:
        ...
        for i, lilora in enumerate(lora_linear_layers):
            state_dict['aux_seed_%d' % i] = lilora.down_aux
            state_dict['down_aux_%d' % i] = lilora.down_aux.data
            state_dict['up_aux_%d' % i] = lilora.up_aux.data
