    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            for i, lilora in enumerate(lora_linear_layers):
                ...
                state_dict['down_aux_%d' % i] = lilora.down_aux
                state_dict['up_aux_%d' % i] = lilora.up_aux
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # weight_prob = 0.1  # Execute weight_loss with a probability of P.
                # rand = torch.rand(1)  # Generate a random number between 0 and 1
                # condition = (rand > weight_prob)
                # loss = torch.where(condition, diffusion_loss, diffusion_loss + weight_loss * args.pre_opt_weight_coeff)

                accelerator.backward(loss)
                if accelerator.sync_gradients:
                    params_to_clip = hypernetwork.train_params()
